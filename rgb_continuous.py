"""
RGB Camera Heatmap + Drowsiness Detection (Continuous Mode)
Runs continuously with no timer.
Left side: RGB camera with face mesh + detection overlays
Right side: Heatmap with heat ratios
Press 's' to hear PERCLOS, blink count, and heat ratios spoken aloud.
Press 'r' to reset head pose.
Press 'q' to quit.
"""
import cv2
...
# Heat ratios (initialized so 's' key works from the start)
hot_ratio = 0.0
warm_ratio = 0.0
cool_ratio = 0.0

print("RGB Continuous Mode running. Press 's' to hear results, 'q' to quit.")
...
while True:
    ...
    # Draw text overlays on camera side
    ...
    # Elapsed time display (counts up instead of down)
    elapsed = time.time() - run_start
    minutes = int(elapsed) // 60
    seconds = int(elapsed) % 60
    cv2.putText(frame, f"Time: {minutes}:{seconds:02d}", (40, 320), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 255), 4)

    cv2.putText(frame, f"FPS: {int(fps)}", (40, img_h - 40), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 4)

    # Calculate heat ratios
    if results.multi_face_landmarks:
    ...
    cv2.imshow("Lumo - RGB Continuous + Heatmap + Detection", combined)
