"""
HD USB Camera - Heatmap + Drowsiness Detection
Combines heatmap visualization with face mesh, EAR eye tracking, and PERCLOS measurement.
Left side: HD camera with face mesh + detection overlays
...
# Initialize HD USB Camera at /dev/video2
cap = cv2.VideoCapture(2)
if not cap.isOpened():
    cap = cv2.VideoCapture(3)
if not cap.isOpened():
    print("Error: Could not open HD USB Camera!")
    exit(1)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
print("HD USB Camera initialized!")
...
print("HD Heatmap + Detection running. Press 'q' to quit, 'r' to reset pose.")
...
while True:
    ...
    cv2.imshow("Lumo - HD Camera + Heatmap + Detection", combined)
